...
// @ts-ignore - Axios types may not be resolved in watch mode
...
export class VehicleQueryService {
  ...
  async queryByPlaca(placa: string): Promise<VehicleQueryResult> {
    try {
      ...
      if (needsApiKey && !apiKey) {
        this.logger.warn('VEHICLE_API_KEY não configurada. Sistema funcionará sem preenchimento automático.');
        return {};
      }
      ...
      try {
        ...
        this.logger.warn(`Erro ao consultar API (${apiProvider}): ${getErrorMessage(apiError)}`);
        return {}; // Retorna vazio para não bloquear o cadastro manual
      }
    } catch (error) {
      this.logger.error(`Erro ao consultar veículo por placa: ${getErrorMessage(error)}`);
  ...
  async queryByRenavan(renavan: string): Promise<VehicleQueryResult> {
    try {
      ...
      this.logger.log(`Consultando dados do veículo pelo RENAVAN: ${cleanRenavan}`);
      ...
      if (freeProviders.includes(apiProvider)) {
        this.logger.warn('Consulta por RENAVAN não suportada pela API gratuita. Use uma API paga.');
        return {};
      }
      ...
      try {
        ...
        if (apiProvider === 'placaapi') {
          ...
          this.logger.warn('Provedor de API não configurado corretamente para RENAVAN.');
          return {};
        }
      ...
      this.logger.error(`Erro ao consultar veículo por RENAVAN: ${getErrorMessage(error)}`);
   ...
   */
  private normalizeResponse(data: any): VehicleQueryResult {
    ...
    // Marca
    result.make = data.marca || data.make || data.brand || data.fabricante || data.manufacturer;

    // Modelo
    result.model = data.modelo || data.model || data.name;

    // Ano
    if (data.ano || data.year || data.anoFabricacao || data.manufactureYear) {
      const yearValue = data.ano || data.year || data.anoFabricacao || data.manufactureYear;
      result.year = typeof yearValue === 'number' ? yearValue : parseInt(String(yearValue), 10);
    ...
    // Cor
    result.color = data.cor || data.color || data.paintColor;

    // VIN/Chassi
    result.vin = data.vin || data.chassi || data.chassis || data.chassiNumber;

    // RENAVAN
    result.renavan = data.renavan || data.renavam || data.renavamNumber;

    // Placa
    result.placa = data.placa || data.plate || data.placaNumber;

    // Combustível
    result.fuelType = data.combustivel || data.fuel || data.fuelType || data.combustivelType;

    // Motor
    result.engine = data.motor || data.engine || data.engineNumber;

    // Chassi (separado do VIN)
    result.chassis = data.chassi || data.chassis || data.chassiNumber;
