...
export class TenantResolverMiddleware implements NestMiddleware {
  ...
  async use(
    ...
    const isPublicOnboardingRoute =
      (pathWithoutQuery === '/api/onboarding/register' && req.method === 'POST') ||
      (pathWithoutQuery === '/api/onboarding/checkout' && req.method === 'POST') ||
      (pathWithoutQuery === '/api/onboarding/webhooks/stripe' && req.method === 'POST') ||
      (pathWithoutQuery === '/api/onboarding/check-status' && req.method === 'POST');
    ...
    if (isPublicTenantRoute || isPublicOnboardingRoute || isPublicEmailRoute || isPublicAuthRoute) {
      this.logger.log(`✅ Rota pública acessada: ${pathWithoutQuery} (${req.method})`);
      return next();
    }

    // Express normaliza headers para lowercase, então 'X-Tenant-Subdomain' vira 'x-tenant-subdomain'
    const headerSubdomain = (req.headers['x-tenant-subdomain'] as string)?.toLowerCase().trim();
    ...
    if (!subdomain || subdomain === 'api' || subdomain === 'www' || subdomain === 'localhost') {
      ...
      if (req.path.includes('/auth/login') || req.path.includes('/auth/refresh')) {
        throw new NotFoundException('Tenant subdomain é obrigatório. Use o header X-Tenant-Subdomain ou subdomain no host.');
      }
      return next();
    }
